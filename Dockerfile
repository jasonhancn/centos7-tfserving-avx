...
RUN ~/.venv/bin/activate && python3 -m pip install --no-cache-dir  "future==0.18.2" "grpcio==1.30.0" "h5py==3.1.0" "keras_applications==1.0.8" "keras_preprocessing==1.1.2" "mock==4.0.3" "numpy==1.19.5" "requests==2.26.0"
...
WORKDIR /serving
RUN ~/.venv/bin/activate && ~/.bazel/bin/bazel build --color=yes --curses=yes \
     --verbose_failures \
